/-
  §3.1 — Non-compositionality of `Strat-NI` (Theorem 1).
...
@[simp] theorem Sc_presL (c : L5) : Sc.presL c = L5.Lo := rfl
...
/-- **Theorem 1** (Non-compositionality).  There are `sA, sB ∈ Strat-NI` whose
